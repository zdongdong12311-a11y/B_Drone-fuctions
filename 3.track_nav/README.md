...
| `published_frame` | `odom` | 发布的位姿所在坐标系 |
| `odom_frame` | `odom` | 里程计坐标系 |
| `provide_odom_frame` | `true` | 由 Cartographer 发布 SLAM 相关 map->odom 变换 |
...
| `static_transform_publisher` | 如有需要，发布 `base_link→body` 等固定变换；`map/odom` 动态 TF 由 Cartographer 等节点负责 |
...
| `local_costmap_params.yaml` | 局部代价地图：`odom` 坐标系，5×5m 滚动窗口，分辨率 0.03m |
...
| 变换 | 发布者 | 说明 |
|------|--------|------|
| `map` → `odom` | Cartographer | SLAM 实时估计的机器人位姿链 |
| `odom` → `base_link` | FAST-LIO2 | 激光雷达里程计 |

> **注意：** 不要额外静态发布 `map→odom`。这些变换由 Cartographer 管理，重复发布会造成 TF 冲突。
